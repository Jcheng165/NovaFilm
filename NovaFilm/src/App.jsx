...
const USER_ID_KEY = 'novafilm_user_id';
const LEGACY_USER_ID_KEY = 'movie_app_user_id';
  ...
  useEffect(() => {
    const initWatchList = async () => {
      try {
        ...
        if (!storeId) {
          const legacyStoreId = localStorage.getItem(LEGACY_USER_ID_KEY);

          if (legacyStoreId) {
            // Migrate old storage naming without losing an existing watchlist.
            storeId = legacyStoreId;
            localStorage.setItem(USER_ID_KEY, legacyStoreId);
            localStorage.removeItem(LEGACY_USER_ID_KEY);
          } else {
            storeId = crypto.randomUUID();
            localStorage.setItem(USER_ID_KEY, storeId);
          }
        }
